...
def run(
    ...
    """MỞ CÁC FILE TXT"""
    pixel_file = open(os.path.join(save_dir, 'pixel.txt'), 'w')
    cm_file = open(os.path.join(save_dir, 'pixel_to_cm.txt'), 'w')
    cm = open(os.path.join(save_dir, 'cm.txt'), 'w')
    cx_pre = 0
    cy_pre = 0
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if (10 < abs(x1-x2) < 80) or (10 < abs(y1-y2) < 80):
                        if abs(cx - cx_pre)<15 or cx_pre==0 or abs(cy - cy_pre)<15 or cy_pre==0:
                            print(conf)
                            temp = 1
                            cx_pre = cx 
                            cy_pre = cy
                ...
                if temp == 1:
                    ...
                    if len(outputs) > 0:
                        ...
                        for j, output in enumerate(outputs):
                            ...
                            pixel_file.write(f"{identity} {cx} {cy}\n")
                                
                            cx_cm = (-660 + cx)*(30/60)
                            cy_cm = (368 - cy)*(30/60)
                            
                            cm_file.write(f"{identity} {cx_cm:.4f} {cy_cm:.4f}\n")
                            cm.write(f"{cx_cm:.4f} {cy_cm:.4f}\n")
                            print(f"{identity} {cx_cm:.4f} {cy_cm:.4f}\n")
...
if __name__ == "__main__":
    run(weights=r"D:\Mobile_Robot\YOLOv9_Object_Tracking\data_ext\weights\best.pt",conf_thres=0.4, source=r"D:\Mobile_Robot\YOLOv9_Object_Tracking\data_ext\Camo Recording 2024-07-30 16-27-20.mp4")
